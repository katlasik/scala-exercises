...
object Comparable {
  ...
  ): Comparable[String] = (left: String, right: String) =>
    left.size
      .compareTo(right.size)
      .pipe(r =>
        if (r > 0) LeftGreater
        else if (r < 0) RightGreater
        else
          LazyList.from(left).lazyZip(right).map(charComparable.compare).dropWhile(_ == Equal).headOption.getOrElse(Equal)
      )
