...
object LoyaltyAccountActor {
  ...
  private case class BehaviourLocked(var points: Points, service: PrizeService, override val context: ActorContext[Command])
    ...
    override def onMessage(msg: Command): Behavior[Command] = msg match {
      case UnblockAccount(replyTo) =>
        context.log.info("Account unblocked!")
        replyTo ! AccountUnblocked
        BehaviourUnlocked(points, service, context)
      case RefundPoints(cost) =>
        points = points + cost
        Behaviors.same
      case e =>
        context.log.info(s"Account blocked, ignoring message: $e")
        Behaviors.same
  ...
  private case class BehaviourUnlocked(var points: Points, service: PrizeService, override val context: ActorContext[Command])
    ...
    override def onMessage(msg: Command): Behavior[Command] = msg match {
      case UnblockAccount(replyTo) =>
        replyTo ! AccountUnblocked
        context.log.info("Account already unlocked!")
        Behaviors.same

      case BlockAccount(replyTo) =>
        replyTo ! AccountBlocked
        context.log.info("Blocking account!")
        BehaviourLocked(points, service, context)

      case AwardPoints(p, replyTo) =>
        points = points + p
        replyTo ! PointsAwarded(points)
        Behaviors.same

      case GetBalance(replyTo) =>
        replyTo ! Balance(points)
        Behaviors.same
      ...
      case OrderPrize(prize, cost, replyTo) =>
        if (points >= cost) {
          points = points - cost
          service.orderPrize(prize).onComplete {
            case Success(_) =>
              replyTo ! PrizeOrdered(prize)
            case Failure(exception) =>
              context.self ! RefundPoints(cost)
              replyTo ! OrderRejected(exception.getMessage)
          }
        } else {
          replyTo ! OrderRejected("Insufficient funds!")
        }
        Behaviors.same
    }
  }
